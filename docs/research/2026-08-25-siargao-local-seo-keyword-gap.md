...
The 2026-08-25 and 2026-08-26 working changes now add explicit service-and-Siargao H1s, a Siargao-focused hardware-supply title, crawlable estimator context, individual solar project URLs, project `BreadcrumbList` markup, responsive project images with intrinsic dimensions, meaningful sitemap `lastmod` dates, and an Organization graph that uses Siargao as the service area. The graph identifies the General Luna office as a non-public `Place`; it does not invent a Burgos or Davao storefront. Contact, footer, and hardware copy label Burgos only as warehouse pickup by arrangement. These changes are local to the repository until deployed; Google, Facebook, Instagram, and directory profiles were not edited during this work.
...
| Sitemap used `priority` but had no `lastmod` (resolved locally on 2026-08-26). | Google says it ignores `priority` and `changefreq`; accurate `lastmod` can help crawl scheduling ([Google Search Central sitemap note](https://developers.google.com/search/blog/2023/06/sitemaps-lastmod-ping)). | P2 |
...
3. Maintain the accurate sitemap `lastmod` values added on 2026-08-26 whenever a listed page changes materially; do not reintroduce `priority`.
...
| **P2** | Post-deployment measurement | Sitemap `<priority>` values have been replaced locally with page-change `<lastmod>` dates, and project pages now have responsive images, intrinsic dimensions, and `BreadcrumbList` markup. After deployment, use Search Console's Core Web Vitals report and PageSpeed Insights on the image-heavy project pages and estimator, then optimize only demonstrated LCP, INP, or CLS problems ([sitemap guidance](https://developers.google.com/search/docs/crawling-indexing/sitemaps/build-sitemap); [Core Web Vitals guidance](https://developers.google.com/search/docs/appearance/core-web-vitals)). | **Medium / low to medium** |

### Local browser spot-check

The estimator is already substantial and crawlable; more generic calculator copy is not the next priority. The 2026-08-26 working changes now label the default electricity rate as an editable planning input rather than a current tariff, label payback as a simple planning calculation, state material exclusions, remove the “25+ years” and carbon/tree-equivalence claims, and clarify that warranty terms must be confirmed in the formal quote. The cost bands remain broad internal planning assumptions and still require a named internal owner to review them whenever the displayed methodology date changes. This supports the reliable, first-hand standard Google asks site owners to apply to people-first content ([people-first content guidance](https://developers.google.com/search/docs/fundamentals/creating-helpful-content)).

A representative new project detail page has a unique title, description, canonical, local H1, visible breadcrumb, equipment specifications, descriptive image alt text, `BreadcrumbList` JSON-LD, and project images with explicit dimensions and responsive WebP sources. Its indexable substance is still mostly template copy and a gallery, so content depth remains the higher-priority follow-up. A representative `primaryImageOfPage` can be considered later if it describes a real page relationship rather than merely adding schema ([breadcrumb documentation](https://developers.google.com/search/docs/appearance/structured-data/breadcrumb); [image SEO guidance](https://developers.google.com/search/docs/appearance/google-images)).

The local browser check is a development baseline, not post-deployment field Core Web Vitals. At a 546 × 1,449 CSS-pixel viewport (device pixel ratio 1.792), the Bamboo Surf project page recorded approximately 260 ms FCP/LCP and 0 CLS; the estimator recorded approximately 116 ms FCP/LCP and 0.014 CLS. These values came from the browser Performance APIs against the local PHP server, so they do not represent real-user network, device, cache, or interaction conditions and do not provide a meaningful field INP. The required follow-up is to measure the deployed URLs with PageSpeed Insights and Search Console's Core Web Vitals report.
